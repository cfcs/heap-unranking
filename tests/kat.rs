    ...
    fn unrank_kats() {
        ...
        /*        assert_eq!(
            ...
            assert_eq!(
                ...
                [4, 6, 1, 11, 0, 2, 7, 5, 9, 8, 3, 10, 12, 13, 14, 15]
            );
            assert_eq!(
                unrank(&s, 16, 63040392),
                [11, 5, 4, 0, 9, 1, 6, 3, 8, 7, 2, 10, 12, 13, 14, 15]
            );
            assert_eq!(
                unrank(&s, 16, 101976303),
                [10, 4, 11, 0, 9, 6, 7, 8, 3, 5, 2, 1, 12, 13, 14, 15]
            );
            assert_eq!(
                unrank(&s, 16, 449569912),
                [3, 9, 8, 10, 11, 1, 7, 2, 6, 4, 5, 0, 12, 13, 14, 15]
            );
            assert_eq!(
                unrank(&s, 16, 453709146),
                [4, 2, 8, 1, 10, 6, 9, 11, 7, 5, 3, 0, 12, 13, 14, 15]
            );
            assert_eq!(
                unrank(&s, 16, 462443280),
                [3, 7, 11, 5, 10, 8, 4, 2, 6, 9, 1, 0, 12, 13, 14, 15]
            );
            assert_eq!(
                unrank(&s, 16, 791652516),
                [0, 2, 12, 11, 6, 7, 3, 4, 1, 10, 8, 5, 9, 13, 14, 15]
            );
            assert_eq!(
                unrank(&s, 16, 1536412922),
                [6, 3, 4, 11, 12, 5, 1, 8, 2, 9, 10, 0, 7, 13, 14, 15]
            );
            assert_eq!(
                unrank(&s, 16, 1719411164),
                [1, 6, 5, 12, 2, 0, 10, 9, 11, 4, 8, 3, 7, 13, 14, 15]
            );
            assert_eq!(
                unrank(&s, 16, 1947450820),
                [12, 2, 0, 3, 5, 1, 10, 7, 11, 9, 4, 8, 6, 13, 14, 15]
            );
            assert_eq!(
                unrank(&s, 16, 2110962693),
                [3, 7, 0, 12, 4, 2, 10, 9, 1, 8, 5, 11, 6, 13, 14, 15]
            );
            assert_eq!(
                unrank(&s, 16, 2168025930),
                [3, 12, 8, 11, 5, 0, 9, 2, 7, 4, 10, 1, 6, 13, 14, 15]
            );
            assert_eq!(
                unrank(&s, 16, 2556786030),
                [6, 10, 8, 3, 12, 1, 4, 9, 2, 11, 7, 0, 5, 13, 14, 15]
            );
        assert_eq!(unrank(&s, 16, 2871869308), [12, 1, 10, 9, 4, 11, 8, 3, 2, 0, 7, 6, 5, 13, 14, 15]);
        assert_eq!(unrank(&s, 16, 3437360017), [4, 6, 5, 11, 2, 0, 12, 9, 8, 1, 10, 7, 3, 13, 14, 15]);
        assert_eq!(unrank(&s, 16, 5167046400), [9, 2, 3, 12, 1, 8, 11, 4, 7, 5, 6, 0, 10, 13, 14, 15]);
        assert_eq!(unrank(&s, 16, 7577732176), [1, 0, 11, 2, 13, 5, 9, 3, 4, 10, 7, 6, 8, 12, 14, 15]);
        assert_eq!(unrank(&s, 16, 10166721673), [13, 0, 11, 4, 9, 7, 8, 10, 1, 3, 5, 6, 2, 12, 14, 15]);
        assert_eq!(unrank(&s, 16, 15479064468), [7, 3, 13, 10, 11, 0, 8, 6, 12, 5, 2, 9, 4, 1, 14, 15]);
        assert_eq!(unrank(&s, 16, 18675635160), [10, 5, 13, 2, 12, 7, 4, 3, 8, 6, 9, 11, 0, 1, 14, 15]);
        assert_eq!(unrank(&s, 16, 21078023364), [9, 12, 13, 4, 3, 10, 6, 1, 0, 11, 8, 7, 5, 2, 14, 15]);
        //  21078023364
        //   3084870208
        //  18612039960
         */

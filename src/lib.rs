...
pub fn precompute(max_n: usize) -> Vec<Box<[u8]>> {
    ...
    for n in 1..max_n {
        ...
        for j in 0..n {
            ...
            reset_permutation(&mut scratch, &s[n - 1], &mut arr);
...
// Runtime: Worst case: n+(n-1)n/2 -> n + 0.5n^2 -> O(n^2)
//          Best case: n+n -> O(n)
//
...
pub fn rank(prefixes: &Vec<Box<[u8]>>, permutation: Box<[u8]>) -> usize {
        ...
        .rev()
    {
        ...
        /* arr[i] != permutation[i] */
        {
            reset_permutation(&mut scratch, prefix /* &s[i-1] */, &mut arr);
            arr.swap((i & 1) * q, i);
            q += 1;
        }
        *qq = q; // qs[i-1] = q;
